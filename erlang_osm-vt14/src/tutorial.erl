...
right_triangles(N) ->
    L = lists:seq(1, N),
    [{A,B,C} || A<-L, B<-L, C<-L,C*C =:= (A*A+B*B),C =< N].
...
char_to_upper(Char) when (97 =< Char andalso Char =< 122)->
    Char-32.
...
char_to_lower(Char) when (65 =< Char andalso Char =< 90)  ->
    Char+32.
...
max([H | T]) ->
    F = fun(M,Y)-> if M > Y -> M; M =< Y -> Y end end,
